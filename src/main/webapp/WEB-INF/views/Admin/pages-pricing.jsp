<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>

<%--  상단 헤더 --%>
<jsp:include page="../partials/main.jsp"></jsp:include>

<head>
   	<%-- 상단 메타정보 --%>
	<jsp:include page="../partials/title-meta.jsp">
	    <jsp:param name="title" value="Pricing" />
	</jsp:include>

	<%-- 상단 라이브러리 --%>
	<jsp:include page="../partials/head-css.jsp"></jsp:include>
</head>

<body>

    <!-- Begin page -->
    <div id="layout-wrapper">
    
		<%-- 메뉴 --%>
		<jsp:include page="../partials/menu.jsp"></jsp:include>
		<%-- 사이드바 --%>
		<jsp:include page="../partials/sidebar.jsp"></jsp:include>

        <!-- ============================================================== -->
        <!-- Start right Content here -->
        <!-- ============================================================== -->
        <div class="main-content">

            <div class="page-content">
                <div class="container-fluid">
                
                   	<%-- 페이지 타이틀 --%>
                   	<jsp:include page="../partials/page-title.jsp">
   						<jsp:param name="pagetitle" value="Pages" />
   						<jsp:param name="title" value="Pricing" />
					</jsp:include>

                    <div class="row">
                        <div class="col-xl-3 col-sm-6">
                            <div class="card mb-xl-0">
                                <div class="card-body">
                                    <div class="p-2">
                                        <h5 class="fs-16">Starter</h5>
                                        <h1 class="mt-3">$29 <span class="text-muted fs-16">/ Month</span></h1>
                                        <p class="text-muted mt-3">For small teams trying out Minia for an unlimited 
                                            period of time</p>
                                        <div class="mt-4 pt-2 text-muted">
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Verifide
                                                work and
                                                reviews</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Dedicated
                                                Ac managers</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Unlimited
                                                proposals</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Project
                                                tracking
                                            </p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Dedicated
                                                Ac managers</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Unlimited
                                                proposals</p>
                                        </div>

                                        <div class="mt-4 pt-2">
                                            <a href="" class="btn btn-outline-primary w-100">Choose Plan</a>
                                        </div>
                                    </div>
                                </div>
                                <!-- end card body -->
                            </div>
                            <!-- end card -->
                        </div>
                        <!-- end col -->

                        <div class="col-xl-3 col-sm-6">
                            <div class="card mb-xl-0">
                                <div class="card-body">
                                    <div class="p-2">
                                        <h5 class="fs-16">Professional</h5>
                                        <h1 class="mt-3">$49 <span class="text-muted fs-16">/ Month</span></h1>
                                        <p class="text-muted mt-3">For larger businesses or those with onal administration needs</p>
                                        <div class="mt-4 pt-2 text-muted">
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Verifide
                                                work and
                                                reviews</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Dedicated
                                                Ac managers</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Unlimited
                                                proposals</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Project
                                                tracking
                                            </p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Dedicated
                                                Ac managers</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Unlimited
                                                proposals</p>
                                        </div>

                                        <div class="mt-4 pt-2">
                                            <a href="" class="btn btn-outline-primary w-100">Choose Plan</a>
                                        </div>
                                    </div>
                                </div>
                                <!-- end card body -->
                            </div>
                            <!-- end card -->
                        </div>
                        <!-- end col -->

                        <div class="col-xl-3 col-sm-6">
                            <div class="card bg-primary mb-xl-0">
                                <div class="card-body">
                                    <div class="p-2">
                                        <div class="pricing-badge">
                                            <span class="badge">Featured</span>
                                        </div>
                                        <h5 class="fs-16 text-white">Enterprise</h5>
                                        <h1 class="mt-3 text-white">$79 <span class="text-white fs-16 fw-medium">/ Month</span></h1>
                                        <p class="text-white-50 mt-3 fs-15">For extra large businesses or those in regulated industries</p>
                                        <div class="mt-4 pt-2 text-white">
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-light fs-18 me-2"></i>Verifide
                                                work and
                                                reviews</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-light fs-18 me-2"></i>Dedicated
                                                Ac managers</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-light fs-18 me-2"></i>Unlimited
                                                proposals</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-light fs-18 me-2"></i>Project
                                                tracking
                                            </p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-light fs-18 me-2"></i>Dedicated
                                                Ac managers</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-light fs-18 me-2"></i>Unlimited
                                                proposals</p>
                                        </div>

                                        <div class="mt-4 pt-2">
                                            <a href="" class="btn btn-light w-100">Choose Plan</a>
                                        </div>
                                    </div>
                                </div>
                                <!-- end card body -->
                            </div>
                            <!-- end card -->
                        </div>
                        <!-- end col -->

                        <div class="col-xl-3 col-sm-6">
                            <div class="card mb-0">
                                <div class="card-body">
                                    <div class="p-2">
                                        <h5 class="fs-16">Unlimited</h5>
                                        <h1 class="mt-3">$99 <span class="text-muted fs-16">/ Month</span></h1>
                                        <p class="text-muted mt-3">For small teams trying out Minia for an unlimited 
                                            period of time</p>
                                        <div class="mt-4 pt-2 text-muted">
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Verifide
                                                work and
                                                reviews</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Dedicated
                                                Ac managers</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Unlimited
                                                proposals</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Project
                                                tracking
                                            </p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Dedicated
                                                Ac managers</p>
                                            <p class="mb-3"><i class="bx bx-check-circle align-middle text-info fs-18 me-2"></i>Unlimited
                                                proposals</p>
                                        </div>

                                        <div class="mt-4 pt-2">
                                            <a href="" class="btn btn-outline-primary w-100">Choose Plan</a>
                                        </div>
                                    </div>
                                </div>
                                <!-- end card body -->
                            </div>
                            <!-- end card -->
                        </div>
                        <!-- end col -->
                    </div>

                    <h4 class="card-title my-4">Project Plans</h4>

                    <div class="card mt-4">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-xl-3">
                                    <div class="nav flex-column nav-pills pricing-tab-box" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                                        <a class="nav-link mb-3 active" id="v-pills-tab-one" data-bs-toggle="pill" href="#v-price-one" role="tab" aria-controls="v-price-one" aria-selected="true">
                                            <div class="d-flex align-items-center">
                                                <i class="bx bx-check-circle h3 mb-0 me-4"></i>
                                                <div class="flex-1">
                                                    <h2 class="fw-medium">$29 <span class="text-muted fs-15">/ Month Plans</span></h2>
                                                    <p class="fw-normal mb-0 text-muted">For small teams trying out Minia for an unlimited period of time</p>
                                                </div>
                                            </div>
                                        </a>
                                        <a class="nav-link mb-3" id="v-pills-tab-two" data-bs-toggle="pill" href="#v-price-two" role="tab" aria-controls="v-price-two" aria-selected="false">
                                            <div class="d-flex align-items-center">
                                                <i class="bx bx-check-circle h3 mb-0 me-4"></i>
                                                <div class="flex-1">
                                                    <h2 class="fw-medium">$79 <span class="text-muted fs-15">/ Month Plans</span></h2>
                                                    <p class="fw-normal mb-0 text-muted">For larger businesses or those with onal administration needs</p>
                                                </div>
                                            </div>
                                        </a>
                                        <a class="nav-link" id="v-pills-tab-three" data-bs-toggle="pill" href="#v-price-three" role="tab" aria-controls="v-price-three" aria-selected="false">
                                            <div class="d-flex align-items-center">
                                                <i class="bx bx-check-circle h3 mb-0 me-4"></i>
                                                <div class="flex-1">
                                                    <h2 class="fw-medium">$99 <span class="text-muted fs-15">/ Month Plans</span></h2>
                                                    <p class="fw-normal mb-0 text-muted">For extra large businesses or those in regulated industries</p>
                                                </div>
                                            </div>
                                        </a>
                                    </div>
                                </div>
        
                                <div class="col-xl-9">
                                    <div class="tab-content text-muted mt-4 mt-xl-0" id="v-pills-tabContent">
                                        <div class="tab-pane fade show active" id="v-price-one" role="tabpanel" aria-labelledby="v-pills-tab-one">
                                            <div class="table-responsive text-center">
                                                <table class="table table-bordered mb-0 table-centered align-middle">
                                                    <tbody>
                                                        <tr>
                                                            <td></td>
                                                            <td style="width: 20%;">
                                                                <div class="py-3">
                                                                    <h5 class="fs-16 mb-0">1 Month</h5>
                                                                </div>
                                                            </td>
                                                            <td style="width: 20%;">
                                                                <div class="py-3">
                                                                    <h5 class="fs-16 mb-0">3 Month</h5>
                                                                </div>
                                                            </td>
                                                            <td style="width: 20%;">
                                                                <div class="py-3">
                                                                    <h5 class="fs-16 mb-0">6 Month</h5>
                                                                </div>
                                                            </td>
                                                            <td style="width: 20%;">
                                                                <div class="py-3">
                                                                    <h5 class="fs-16 mb-0">1 Year</h5>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Users</th>
                                                            <td>1</td>
                                                            <td>3</td>
                                                            <td>5</td>
                                                            <td>7</td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Storage</th>
                                                            <td>1 GB</td>
                                                            <td>10 GB</td>
                                                            <td>20 GB</td>
                                                            <td>40 GB</td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Domain</th>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-close-circle text-danger fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>1</td>
                                                            <td>2</td>
                                                            <td>4</td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Hidden Fees</th>
                                                            <td>Yes</td>
                                                            <td>Yes</td>
                                                            <td>No</td>
                                                            <td>No</td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Support</th>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-close-circle text-danger fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Update</th>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-close-circle text-danger fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-close-circle text-danger fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
                                        </div>
        
                                        <div class="tab-pane fade" id="v-price-two" role="tabpanel" aria-labelledby="v-pills-tab-two">
                                            <div class="table-responsive text-center">
                                                <table class="table table-bordered mb-0 table-centered align-middle">
                                                    <tbody>
                                                        <tr>
                                                            <td></td>
                                                            <td style="width: 20%;">
                                                                <div class="py-3">
                                                                    <h5 class="fs-16 mb-0">1 Month</h5>
                                                                </div>
                                                            </td>
                                                            <td style="width: 20%;">
                                                                <div class="py-3">
                                                                    <h5 class="fs-16 mb-0">3 Month</h5>
                                                                </div>
                                                            </td>
                                                            <td style="width: 20%;">
                                                                <div class="py-3">
                                                                    <h5 class="fs-16 mb-0">6 Month</h5>
                                                                </div>
                                                            </td>
                                                            <td style="width: 20%;">
                                                                <div class="py-3">
                                                                    <h5 class="fs-16 mb-0">1 Year</h5>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Users</th>
                                                            <td>1</td>
                                                            <td>3</td>
                                                            <td>5</td>
                                                            <td>7</td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Storage</th>
                                                            <td>5 GB</td>
                                                            <td>15 GB</td>
                                                            <td>25 GB</td>
                                                            <td>50 GB</td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Domain</th>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-close-circle text-danger fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>3</td>
                                                            <td>4</td>
                                                            <td>8</td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Hidden Fees</th>
                                                            <td>Yes</td>
                                                            <td>No</td>
                                                            <td>No</td>
                                                            <td>No</td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Support</th>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Update</th>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-close-circle text-danger fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
                                        </div>
        
                                        <div class="tab-pane fade" id="v-price-three" role="tabpanel" aria-labelledby="v-pills-tab-three">
                                            <div class="table-responsive text-center">
                                                <table class="table table-bordered mb-0 table-centered align-middle">
                                                    <tbody>
                                                        <tr>
                                                            <td></td>
                                                            <td style="width: 20%;">
                                                                <div class="py-3">
                                                                    <h5 class="fs-16 mb-0">1 Month</h5>
                                                                </div>
                                                            </td>
                                                            <td style="width: 20%;">
                                                                <div class="py-3">
                                                                    <h5 class="fs-16 mb-0">3 Month</h5>
                                                                </div>
                                                            </td>
                                                            <td style="width: 20%;">
                                                                <div class="py-3">
                                                                    <h5 class="fs-16 mb-0">6 Month</h5>
                                                                </div>
                                                            </td>
                                                            <td style="width: 20%;">
                                                                <div class="py-3">
                                                                    <h5 class="fs-16 mb-0">1 Year</h5>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Users</th>
                                                            <td>1</td>
                                                            <td>3</td>
                                                            <td>5</td>
                                                            <td>7</td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Storage</th>
                                                            <td>5 GB</td>
                                                            <td>30 GB</td>
                                                            <td>50 GB</td>
                                                            <td>100 GB</td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Domain</th>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>3</td>
                                                            <td>5</td>
                                                            <td>10</td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Hidden Fees</th>
                                                            <td>No</td>
                                                            <td>No</td>
                                                            <td>No</td>
                                                            <td>No</td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Support</th>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <th scope="row">Update</th>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <div>
                                                                    <i class="mdi mdi-check-circle text-success fs-20"></i>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                </div>
                <!-- container-fluid -->
            </div>
            <!-- End Page-content -->
            
			<%-- 푸터 --%>
			<jsp:include page="../partials/footer.jsp"></jsp:include>

        </div>
        <!-- end main content-->

    </div>
    <!-- END layout-wrapper -->
    
	<%-- 커스터마이징 --%>
	<jsp:include page="../partials/customizer.jsp"></jsp:include>
	
	<%-- 자바스크립트 --%>
	<jsp:include page="../partials/vendor-scripts.jsp"></jsp:include>

    <!-- App js -->
    <script src="/resources/bootstrap/js/app.js"></script>
</body>

</html>